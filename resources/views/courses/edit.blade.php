<x-app-layout>
    <div class="w-8/12 mx-auto">
    
<form action="{{route('courses.update', $course->id)}}" method="POST">
    @csrf
    @method('PUT')


    <div>
            <h2 class="text-3xl font-bold text-gray-800 mb-2 mt-6">Edit Course</h2>
            <p class="text-gray-600 mb-6">Use the sidebar to navigate through your pages.</p>
            </div>

      <!-- Name -->
        <div>
            <x-input-label for="name" :value="__('Name')" />
            <x-text-input id="name" value="{{ old('name',$course->name) }}" class="block mt-1 w-full py-3 bg-white" type="text" name="name"  required autofocus autocomplete="name" />
            <x-input-error :messages="$errors->get('name')" class="mt-2" />
        </div>

        <!-- Course Category -->
               <div class="mt-4">
            <x-input-label for="category" :value="__('Program_id')" />
            
                             <div class="mt-4">
                                <select name="category_id" id="category_id" class="block mt-1 w-full py-3 bg-white rounded-md shadow-sm focus:ring focus:ring-indigo-200" :value="old('category_id')" required autocomplete>
                                    <option value="">Select Program</option>
                                    
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}" @selected($category->id == $course->category_id)>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                               

                                @error('category_id')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                                @enderror
                            </div>
                       
        </div>

          <div class="mt-4">
                                <x-input-label for="academic_year_id" :value="__('Academic_Year')" />
                                <select name="academic_year_id" id="academic_year_id" class="block mt-1 w-full py-3 bg-white rounded-md shadow-sm focus:ring focus:ring-indigo-200" :value="old('academic_year_id')" required autocomplete>
                                    <option value="">Select Class</option>
                                    
                                    @foreach($classes as $class)
                                        <option value="{{$class->id}}" @selected($class->id == $course->academic_year_id)>{{$class->name}}</option>
                                        @endforeach
                                </select>
          </div>
        



           <x-primary-button class="ms-4 mt-6">
                {{ __('Submit') }}
            </x-primary-button>
    </form>
    </div>
 </form>

</div>
 </x-app-layout>











