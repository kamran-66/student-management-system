...
<form action="{{route('sections.update', $sections->id)}}" method="POST">
    ...
    <div>
            <h2 class="text-3xl font-bold text-gray-800 mb-2 mt-6">Edit Section</h2>
      ...
      <!-- Name -->
        <div>
            ...
            <x-text-input id="name" value="{{ old('name',$sections->name) }}" class="block mt-1 w-full py-3 bg-white" type="text" name="name"  required autofocus autocomplete="name" />
          ...
          {{-- <div class="mt-4">
                                ...
                                <select name="course_id" id="_id" class="block mt-1 w-full py-3 bg-white rounded-md shadow-sm focus:ring focus:ring-indigo-200" :value="old('course_id')" required autocomplete>
                                    ...
                                    @foreach($courses as $course)
                                        <option value="{{$course->id}}"  @selected($sections->course_id==$course->id)>{{$course->name}}</option>
                                        @endforeach
                                </select>
          </div> --}}
